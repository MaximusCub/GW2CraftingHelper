...
at all 3 real call sites in `CraftingPlanPipeline` (single-item,
batch, and the pre-pass's own re-run inside `ResolveWithOverrides`). Test:
...
none was specified); extracting the now-three-times-duplicated
`NonLevelableDisciplineTags`/`NonCraftingDisciplines`/
`InherentlyAvailableDisciplines` set (flagged for a future pass, not
this one); the persisted-plan JSON size of the 3 new
`PillSourceCostBreakdown`s per node (no measurement taken, no
`[JsonIgnore]` added).
