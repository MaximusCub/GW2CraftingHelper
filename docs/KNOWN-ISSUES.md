...
10-character tag allowance - rather than from the rows currently on
...
Worst case is still bounded by the ring's 2000 entries.
...
full lines - unaffected by the split). `Services/LogRowLayout` carries
...
never-collapse floor) -> 1900 after commit 2 (view-only).
...
2. Narrowing and widening the module window re-fits the rows: the
   ...
   appears/disappears with the truncation.
