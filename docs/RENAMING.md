...
`taimis-toolbench` branch (see the CHANGELOG 0.3.0 entry). This file is
the runbook for everything the content sweep could NOT do: steps that
touch GitHub, the local dev environment, and the owner's live Blish HUD
install. Execute top to bottom, after both of these have merged:

1. PR #187 (the open README PR - reconcile its README changes with the
   renamed README after it merges),
2. the rename PR itself.
...
holding handles under the folder first.
...
Re-create any junction that pointed at the old folder path (the
`wt-rename` worktree had one; delete the worktree instead once the PR
merges - `git worktree remove /mnt/c/Dev/Blish/wt-rename`). For a
future worktree:
...
Out-of-repo scripts that embed the old repo path (measured on
...
`GW2CraftingHelper.bhm` module paths) in those to the new names.
...
- `data\` - the module's data directory (plan.json, plan_history,
  ranker.json, recipe_cache, snapshot.json, module_log.jsonl, ...).
  NOT keyed by namespace: the manifest registers the directory NAME
  `data` (`"directories": ["data"]`), and Blish creates it directly
  under `addons\blishhud\`. The renamed manifest registers the same
  name, so this directory carries over with NO migration. Do not move
  or copy anything here.
...
Optional tidy: `WindowSettings2` in `settings.json` holds an orphaned
entry keyed `GW2CraftingHelper_ModalDialog_c4f19a` (a saved dialog
position; the renamed code writes `TaimisToolbench_ModalDialog_...`
keys instead). Harmless to leave; delete the entry if tidying.
