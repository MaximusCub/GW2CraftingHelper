...
Scope expansion (called out in the PR body, not silently absorbed): three
...
outside this feature's original scope (those three sites were already
date-formatted) and is called out in the PR body rather than silently
included.
...
**OPEN DECISION (layout risk, not silently patched around).** At the
...
row, or accept the risk as-is - left open.
